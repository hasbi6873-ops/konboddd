"""
Bot Scalping v20.8 — EXHAUSTION BLOCKED
====================================================
Data aktual v20.7 (674T, ~270 jam):
  EXHAUSTION    : 411T, WR=54.5%, BEP=58.0%, EV=-0.0098U/T → -0.357U/HARI
  TRENDING_BULL : 158T, WR=63.3%, BEP=58.5%, EV=+0.013U/T  → +0.184U/HARI
  TRENDING_BEAR : 105T, WR=61.0%, BEP=60.1%, EV=+0.002U/T  → +0.020U/HARI

Keputusan: BLOKIR EXHAUSTION sepenuhnya.
Alasan:
  - Sudah dicoba 3x perbaikan (ec>=2, ec>=3, MIN_SCORE 55→65→70)
  - WR malah turun dari 58% (v20.6) ke 55% (v20.7) setelah difilter
  - EXHAUSTION = mean reversion → tidak cocok dengan trailing stop
    (harga reversal cenderung pendek, avg_peak 0.565% tp WR jelek 54.5%)
  - Bleeding -0.357U/hari tidak bisa diperbaiki dengan filter lebih ketat

Setelah blokir EXHAUSTION:
  - Frekuensi: 2.5T/hr → 0.97T/hr (turun ~60%)
  - EV/hari: -0.152U → +0.205U (flip profitable)
  - Hanya TRENDING_BULL + TRENDING_BEAR yang masuk (yang sudah terbukti +)

Diblokir (sama seperti RANGE + VOLATILE): EXHAUSTION
Diizinkan: TRENDING_BULL, TRENDING_BEAR
"""
...
# Scoring & Filter
MIN_SCORE      = 55   # threshold untuk TRENDING_BULL + TRENDING_BEAR (EXHAUSTION diblokir v20.8)
...
class SignalScorer:
    ...
    def get_signal(self, df: pd.DataFrame, symbol: str = None):
        ...
        if regime == MarketRegime.REGIME_TRENDING_BULL:
            ...
            # v20.8: EXHAUSTION DIBLOKIR sepenuhnya.
            # Data 411 trade: WR=54.5% vs BEP=58.0% → EV=-0.0098U/trade = -0.357U/hari
            # Sudah dicoba filter 3x (ec>=2, >=3, MIN_SCORE 55/65/70) — tidak membaik.
            # Mean reversion + trailing stop = kombinasi buruk (run terlalu pendek).
            _stats["regime_block"] += 1
            return None, max(long_score, short_score), [], atr, 0, 0, regime, bias
...
def print_inline():
    ...
    e   = "💚" if pnl >= 0 else "🔴"
    print(f"       ┌ [v20.8] {n}T WR:{wr:.0f}% W:{_stats['wins']} L:{_stats['losses']} {e}PnL:{pnl:+.4f}U")
...
def print_full():
    ...
    print(f"\n  {'─'*70}")
    print(f"    🔔 TRAIL v20.8 — TRENDING ONLY (EXHAUSTION BLOCKED)")
    ...
    print(f"    ⚙️  SL:{SL_PCT*100:.2f}% | Trail@{TRAIL_ACTIVATE_PCT*100:.2f}% gap:{TRAIL_GAP_PCT*100:.2f}% | MinScore:{MIN_SCORE} (TREND only — EXHAUSTION blocked)")
...
def run_bot():
    print("╔════════════════════════════════════════════════════════════════════╗")
    print("║  🔔 TRAIL v20.8 — TRENDING ONLY (EXHAUSTION BLOCKED)                          ║")
    print(f"║  ✅ SL:{SL_PCT*100:.2f}% | Trail@{TRAIL_ACTIVATE_PCT*100:.2f}% gap:{TRAIL_GAP_PCT*100:.2f}%                                ║")
    print(f"║  ✅ Hanya TRENDING_BULL + TRENDING_BEAR (RANGE+VOLATILE+EXHAUSTION blocked)    ║")
    print("║  ✅ EXHAUSTION BLOCKED: -0.357U/hari dihentikan (411T data, WR=54.5%)   ║")
    print("║  ✅ TREND only: +0.205U/hari (vs -0.152U/hari saat EXHAUSTION aktif)      ║")
    print("╚════════════════════════════════════════════════════════════════════╝")
    try:
